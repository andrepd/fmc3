...
do for [i=9:9]{ #O valor i vai do grupo de picos inicial ao grupo final
	...
	if (number_of_peaks==2) {
		F(x)=V(x,x01,w1,eta1,A1)+V(x,x02,w2,eta2,A2)+b

		fit [min:max] F(x) 'patt.dat' via A1,A2,b
		fit [min:max] F(x) 'patt.dat' via w1,w2
		fit [min:max] F(x) 'patt.dat' via eta1,eta2
		fit [min:max] F(x) 'patt.dat' via A1,x01,w1,eta1, A2,x02,w2,eta2,b
		...
		if (number_of_peaks==4) {
			F(x)=V(x,x01,w1,eta1,A1)+V(x,x02,w2,eta2,A2)+\
				V(x,x03,w3,eta3,A3)+V(x,x04,w4,eta4,A4)+b

			
			fit [min:max] F(x) 'patt.dat' via A1,A2,A3,A4,b
			fit [min:max] F(x) 'patt.dat' via w1,w2,w3,w4
			fit [min:max] F(x) 'patt.dat' via eta1,eta2,eta3,eta4
			fit [min:max] F(x) 'patt.dat' via A1,x01,w1,eta1, A2,x02,w2,eta2,\
				A3,x03,w3,eta3, A4,x04,w4,eta4,b
			...
			F(x)=V(x,x01,w1,eta1,A1)+V(x,x02,w2,eta2,A2)+V(x,x03,w3,eta3,A3)+\
				V(x,x04,w4,eta4,A4)+V(x,x05,w5,eta5,A5)+V(x,x06,w6,eta6,A6)+b

			fit [min:max] F(x) 'patt.dat' via A1,A2,A3,A4,A5,A6,b
			fit [min:max] F(x) 'patt.dat' via eta1,eta2,eta3,eta4,eta5,eta6
			fit [min:max] F(x) 'patt.dat' via w1,w2,w3,w4,w5,w6
			fit [min:max] F(x) 'patt.dat' via A1,x01,w1,eta1, A2,x02,w2,eta2,\
				A3,x03,w3,eta3, A4,x04,w4,eta4, A5,x05,w5,eta5, A6,x06,w6,eta6,b
